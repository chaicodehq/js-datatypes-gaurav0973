...
export function processRailwayPNR(pnrData) {
  ...
  const passengers = pnrData.passengers.map(p => {
    ...
    if (curr === "CAN") statusLabel = "CANCELLED";
    else if (curr.startsWith("WL")) statusLabel = "WAITING";
    else if (curr.startsWith("RAC")) statusLabel = "RAC";
    else if (curr.startsWith("B") || curr.startsWith("S")) statusLabel = "CONFIRMED";
  ...
  const counts = passengers.reduce((acc,p)=>{
    acc[p.statusLabel.toLowerCase()]++;
    return acc;
  },{confirmed:0,waiting:0,cancelled:0,rac:0});
